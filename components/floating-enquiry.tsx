...
export function FloatingEnquiry() {
  ...
  return (
    <div className="fixed right-3.5 bottom-[max(1rem,env(safe-area-inset-bottom))] z-40 flex items-center gap-2 sm:right-6 sm:bottom-6 sm:gap-2.5">
      {/* WhatsApp Quick Chat */}
      <a
        href={WHATSAPP_URL}
        target="_blank"
        rel="noopener noreferrer"
        className="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-600 text-white shadow-lg transition-transform hover:scale-105 active:scale-95 sm:h-12 sm:w-12"
      ...
      <button
        type="button"
        onClick={() => openEnquiry()}
        className="float-enquire flex items-center gap-1.5 rounded-full bg-gold px-4 py-2.5 text-[0.72rem] font-bold tracking-[0.1em] text-navy-deep uppercase shadow-lg transition-transform hover:scale-105 active:scale-95 sm:px-5 sm:py-3 sm:text-xs"
      >
